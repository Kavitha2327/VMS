...
//   connection = mysql.createConnection(db_config_users);

//   connection.connect(function(err) {
//     if (err) {
//       console.log('error when connecting to db (users):', err);
//       setTimeout(handleDisconnect, 2000);
//     }
//   });

//   connection.on('error', function(err) {
//     console.log('db error (users)', err);
...
function First_Job() {
  vehicletripdata.find({}, function (err, docs) {
    ...
    if (!docs || docs.length === 0) {
      // No trip data found — skipping update
    } else {
...
function Rta() {
  branchvehicle.find({}, function (err, docs) {
    ...
    var lengths1 = docs ? docs.length : 0;
    if (!docs || lengths1 === 0) {
      // No branchvehicle data found — skipping RTA update
...
function Pollution() {
  branchvehicle.find({}, function (err, docs) {
    ...
    var lengths = docs ? docs.length : 0;
    if (!docs || lengths === 0) {
      // No branchvehicle data found — skipping Pollution update
    } else {
      for (i = 0; i < lengths; i++) {
        //  console.log(docs[i])
        var branch = docs[i].branch;
...
function Fitness() {
  branchvehicle.find({}, function (err, docs) {
    ...
    var lengths1 = docs ? docs.length : 0;
    if (!docs || lengths1 === 0) {
      // No branchvehicle data found — skipping Fitness update
    } else {
      for (i = 0; i < lengths1; i++) {
        //  console.log(docs[i])
        var branch = docs[i].branch;
...
function Vehiclechallan() {
  branchvehicle.find({}, function (err, docs) {
    ...
    var lengths3 = docs ? docs.length : 0;
    if (!docs || lengths3 === 0) {
      // No branchvehicle data found — skipping Vehiclechallan update
    } else {
...
function Insuranceclaim() {
  branchvehicle.find({}, function (err, docs) {
    ...
    var lengths4 = docs ? docs.length : 0;
    if (!docs || lengths4 === 0) {
      // No branchvehicle data found — skipping Insuranceclaim update
    } else {
...
function Roadtax() {
  branchvehicle.find({}, function (err, docs) {
    ...
    var lengths6 = docs ? docs.length : 0;
    if (!docs || lengths6 === 0) {
      // No branchvehicle data found — skipping Roadtax update
    } else {
