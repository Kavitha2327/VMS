...
router.get('/getBusfilldata', function(req,res){
  ...
  if(req.session && req.session.user){
    ...
    //console.log(req.session.user.branch)
      ...
      //       busfill.find({"date":data.date}, function(err,docs){
      //         res.send(docs)
      //         console.log(docs)
      //         });           
      //     },
...
router.post('/gettingvehicletripdata', function(req,res){
  ...
  if(req.session.user.username=='vms') {
      ...
      vehicletripdata.find({"branch":{$in:["ADITYA PUBLIC SCHOOL (SRI NAGAR)-AA","ADITYA PUBLIC SCHOOL (SRI NAGAR)-SES","ADITYA PUBLIC SCHOOL \n(SRI NAGAR)-SES","ADITYA PUBLIC SCHOOL \n(SRI NAGAR)-AA"]}, "Timestamp":{$gte:ftime,$lte:ttime}}, function(err,docs){
      res.send(docs)
      });
    }
    else if (req.session.user.username=='ajckkd') {
