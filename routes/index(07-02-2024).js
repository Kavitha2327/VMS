...
router.get('/getStagedata', function(req,res){
  if(req.session && req.session.user){
    ...
    if (req.session.user.username=='vms' || req.session.user.username=='vmskkd' || req.session.user.username=='vc') {
      ...
      branch.find({"branch":{$in:["MARIKAVALASA-AA","MARIKAVALASA-SES"]}}, function(err,docs){
      res.send(docs)
      });
    }
    else if (req.session.user.username=='adcho') {
      branch.find({"branch":/KAKINADA HEAD OFFICE/}, function(err,docs){
      res.send(docs)
      });
    }
    else if (req.session.user.username=='adckkdiit') {
      branch.find({"branch":{$in:["KKD IIT-SES","KKD IIT-AA"]}}, function(err,docs){
      res.send(docs)
      });
    }
    else if (req.session.user.username=='adcadmin') {
      branch.find({"branch":{$in:["SURAMPALEM-ENGINEERING-AA","SURAMPALEM-ENGINEERING-SES"]}}, function(err,docs){
      res.send(docs)
      });
    }
...
router.post('/gettingvehicletripdata', function(req,res){
  ...
  if(req.session.user.username=='vms') {
      ...
      vehicletripdata.find({"branch":{$in:["KKD ENGINEERING-AA","KKD ENGINEERING-SES","NON LOCAL ENGINEERING-AA","NON LOCAL ENGINEERING-SES","RJY ENGINEERING-AA",
      "RJY ENGINEERING-SES","MANDAPETA ENGINEERING-AA","MANDAPETA ENGINEERING-SES"]}, "Timestamp":{$gte:ftime,$lte:ttime}}, function(err,docs){
      res.send(docs)
      });
    }
    else if (req.session.user.username=='adckkd') {
    ...
    if (req.session.user.username=='vms' || req.session.user.username=='vmskkd' || req.session.user.username=='vc') {
      vehicleservice.find({"vehicleregno":req.body.val}, function(err,docs){
        ...
        for(i=0;i<docs.length;i++){
         ...
         // vehicletripdata.find({"regno":vehicleregno,"date":{$lte: dates}}, function(err1,docs1){
          vehicletripdata.findOne({"regno":vehicleregno},{ sort: { Timestamp: -1 } }, function(err1,docs1){
           // console.log(docs)
          //  if(docs1.regno==undefined || docs1.regno==0){
          //   console.log("dd")
          // }          
...
router.post('/gettingvehiclebranchdetails', function(req, res){
  ...
  busfill.findOne(
    ...
    (err1, data) => {
      console.log(data);
      branchvehicle.findOne({"vehicleregno": req.body.value}, function(err, docs){
        if(docs === null || docs.length === undefined || docs.length === 0){
          res.send(docs);
        } else {
          docs.omr = data ? data.cmr : "0";
          //  console.log(docs);
          res.send(docs);
        }
      });
